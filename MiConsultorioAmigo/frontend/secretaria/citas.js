document.addEventListener("DOMContentLoaded", function () {
  ...
  const btnCreate = document.querySelector("#create-citas");

  btnCreate.addEventListener("click", () => {
    console.log("hola");
    document.querySelector("#create-citas-form").style.display = "block";
  });
  ...
  function Citas() {
    ...
    const createForm = document.querySelector("#create-citas-form");
    const estadoInputC = document.querySelector("#create-citas-estado");
    const fechaInputC = document.querySelector("#create-citas-fecha");
    const medicoInputC = document.querySelector("#create-citas-medico");
    const pacienteInputC = document.querySelector("#create-citas-paciente");
    const duracionInputC = document.querySelector("#create-citas-duracion");
    const comentariosInputC = document.querySelector(
      "#create-citas-comentarios"
    );
    const motivoInputC = document.querySelector("#create-citas-motivo");
    ...
    createForm.addEventListener("submit", (event) => {
      event.preventDefault();
      const estado = estadoInputC.value;
      const fecha = fechaInputC.value;
      const medico = medicoInputC.value;
      const paciente = pacienteInputC.value;
      const duracion = duracionInputC.value;
      const comentarios = comentariosInputC.value;
      const motivo = motivoInputC.value;

      fetch(`http://localhost:8080/cita/crear`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      row.innerHTML = `
            ...
            <div style="display:flex; gap: 10px">
              <button class="btn btn-primary update-cita" data-id="${
                cita.id
              }" data-toggle="modal" data-target="#editarCita">Actualizar</button>
    ...
    const selectCreateM = document.querySelector("#create-citas-medico");
    medicos.forEach((esp) => {
      const option = document.createElement("option");
      option.value = esp.id;
      option.text = esp.nombre;
      selectCreateM.appendChild(option);
    });
    ...
    const selectCreateP = document.querySelector("#create-citas-paciente");
    pacientes.forEach((esp) => {
      const option = document.createElement("option");
      option.value = esp.id;
      option.text = esp.nombre;
      selectCreateP.appendChild(option);
    });
  }
});
